...
Виклик сеттера (наприклад, `setCount(nextCount)` або `setReady(nextReady)`)
...
Крім того, ви можете встановити `equals` у значення `false`, щоб завжди повторно
...
У Solid не завжди потрібно обгортати функції у `memo`;
